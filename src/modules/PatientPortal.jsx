...
// ── MoodSection ───────────────────────────────────────────────────────────────
const MOOD_EMOJIS = ["😢","😞","😔","😟","😐","🙂","😊","😄","😁","🤩"];
const MOOD_LABELS = ["Muy mal","Mal","Bastante mal","No muy bien","Regular","Bien","Bastante bien","Muy bien","Excelente","¡Increíble!"];

function MoodSection({ phone }) {
  ...
  const todayLog  = logs.find(l => l.logged_at?.startsWith(todayStr));

  const handleSave = async () => {
    if (!score) return;
    setSaving(true); setError("");
    try {
      await saveMoodLog({ phone, score, note });
      const updated = await getMoodLogsByPhone(phone);
      setLogs(updated || []);
      setSaved(true);
      setScore(null);
      setNote("");
  ...
  // Mini gráfica de barras (últimos 7 días)
  const last7 = (() => {
    const days = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
    ...
    <div style={{ fontFamily:P.fB }}>
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ fontSize:13, color:P.tm }}>
          Un registro diario ayuda a tu psicólogo(a) a entender tu progreso.
        </div>
      </div>
      ...
      {saved ? (
        <div style={{
          background:P.sucA, borderRadius:16, padding:"20px",
          marginBottom:16, textAlign:"center",
          border:`1.5px solid ${P.suc}40`,
        }}>
          <div style={{ fontSize:48, marginBottom:8 }}>✅</div>
          <div style={{ fontFamily:P.fB, fontSize:15, fontWeight:700, color:P.suc, marginBottom:4 }}>
            ¡Registro guardado!
          </div>
          <div style={{ fontSize:13, color:P.tm }}>Vuelve mañana para tu próximo registro.</div>
        ...
        <div style={{
          background:P.pA, borderRadius:16, padding:"18px",
          marginBottom:16, border:`1.5px solid ${P.p}30`,
          display:"flex", alignItems:"center", gap:14,
        }}>
          <div style={{ fontSize:40 }}>{MOOD_EMOJIS[todayLog.mood_score - 1]}</div>
          <div>
            <div style={{ fontSize:11, fontWeight:700, color:P.p, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:2 }}>
              Registro de hoy
            </div>
            <div style={{ fontFamily:P.fH, fontSize:20, fontWeight:600, color:P.t }}>
              {todayLog.mood_score}/10 · {MOOD_LABELS[todayLog.mood_score - 1]}
            </div>
            {todayLog.note && (
              <div style={{ fontSize:13, color:P.tm, marginTop:4, fontStyle:"italic" }}>
                "{todayLog.note}"
              </div>
            )}
          </div>
        ...
        <div style={{
          background:P.card, borderRadius:16, padding:"20px",
          marginBottom:16, boxShadow:P.sh, border:`1.5px solid ${P.bdrL}`,
        }}>
          <div style={{ fontSize:13, fontWeight:700, color:P.t, marginBottom:16 }}>
            ¿Cómo te sientes hoy?
          </div>

          {/* Selector de emoji/score */}
          <div style={{ display:"flex", gap:4, justifyContent:"center", flexWrap:"wrap", marginBottom:12 }}>
            {MOOD_EMOJIS.map((emoji, i) => {
              const val = i + 1;
              const active = score === val;
              return (
                <button key={val} onClick={() => setScore(val)}
                  style={{
                    width:40, height:40, borderRadius:10, border:"none",
                    background: active ? P.p : P.bdrL,
          ...
          {score && (
            <div style={{ textAlign:"center", marginBottom:14 }}>
              <span style={{ fontFamily:P.fH, fontSize:18, fontWeight:600, color:P.p }}>
                {score}/10 · {MOOD_LABELS[score - 1]}
              </span>
            </div>
          )}
          ...
          <textarea
            ...
            placeholder="Agrega una nota opcional... ¿qué influyó en tu estado de ánimo hoy?"
            rows={2}
            style={{
              width:"100%", padding:"10px 12px", borderRadius:10,
              border:`1.5px solid ${P.bdr}`, background:P.bg,
              fontFamily:P.fB, fontSize:13, color:P.t,
              resize:"none", outline:"none", boxSizing:"border-box",
              marginBottom:12,
            }}
          ...
          {error && (
            <div style={{ fontSize:12, color:P.err, marginBottom:8 }}>{error}</div>
          )}

          <button onClick={handleSave} disabled={!score || saving}
            style={{
              width:"100%", padding:"12px", borderRadius:12, border:"none",
              background: score && !saving ? P.p : P.bdr,
              color: score && !saving ? "#fff" : P.tl,
              fontFamily:P.fB, fontSize:14, fontWeight:700,
              cursor: score && !saving ? "pointer" : "not-allowed",
              display:"flex", alignItems:"center", justifyContent:"center", gap:8,
              transition:"all .15s",
            }}>
            {saving
              ? <><Loader size={14} style={{ animation:"spin 0.8s linear infinite" }}/> Guardando...</>
          ...
          <div style={{ display:"flex", alignItems:"flex-end", gap:6, height:80 }}>
            {last7.map((d, i) => (
              <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                <div style={{ fontSize:10, color:P.tl }}>
                  {d.score ? `${d.score}` : "—"}
                </div>
                <div style={{
                  width:"100%", borderRadius:"4px 4px 0 0",
                  background: d.score
                    ? d.score >= 7 ? P.suc : d.score >= 4 ? P.p : P.err
                    : P.bdrL,
                ...
                <div style={{
                  fontSize:9, color: d.isToday ? P.p : P.tl,
          ...
          {logs.map((log, i) => {
            ...
            return (
              <div key={log.id || i} style={{
                background:P.card, borderRadius:12, padding:"12px 16px",
                marginBottom:8, border:`1px solid ${P.bdrL}`,
                display:"flex", alignItems:"center", gap:12,
              }}>
                <div style={{ fontSize:26 }}>{MOOD_EMOJIS[log.mood_score - 1]}</div>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:2 }}>
                    <span style={{ fontFamily:P.fB, fontSize:14, fontWeight:700, color:P.t }}>
                      {log.mood_score}/10
                    </span>
                    <span style={{ fontSize:12, color:P.tm }}>{MOOD_LABELS[log.mood_score - 1]}</span>
                  </div>
                  <div style={{ fontSize:11, color:P.tl }}>{date}</div>
                  {log.note && (
                    <div style={{ fontSize:12, color:P.tm, marginTop:3, fontStyle:"italic" }}>
                      "{log.note}"
                    </div>
                  )}
                </div>
